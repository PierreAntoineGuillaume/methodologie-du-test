...
class FilesystemModelComparator(ModelComparator):
    def __init__(self, wrapped: InMemoryModelComparator, json_path: str):
            ...
            with open(self.json_path, "r") as file:
                ...
                for item in data:
                    self.wrapped.add_metadata(
                        Metadata(name=item["name"], mse=item["mse"])
                    )
    ...
    def _save_metadata(self) -> None:
        with open(self.json_path, "w") as file:
            json.dump(
                [{"name": m.name, "mse": m.mse} for m in self.wrapped.metadata_list],
                file,
            )
